...
from shop.models import Bicycle


class Basket:
    def __init__(self, requests):
        self.session = requests.session
        basket = self.session.get(settings.BACKET_SESSION_ID)
        if not basket:
            basket = self.session[settings.BACKET_SESSION_ID] = {}
        self.basket = basket
    
    def __iter__(self):
        product_ids = self.basket.keys()
        product_list = Bicycle.objects.filter(pk__in=product_ids)
    ...
    def save(self):
        self.session[settings.BACKET_SESSION_ID] = self.basket
    ...
    def clear(self):
        del self.session[settings.BACKET_SESSION_ID]
        self.session.modified = True